...
\textbf{VERIFICATION:}
\begin{checklist}
    \item \osd{LAC ARMED} indicator appears (CYAN/BLUE)
    \item System latches current target tracking rate
    \item Track confidence remains >70\%
\end{checklist}
\end{procedurebox}

\subsubsection{LAC ARM → ENGAGE → DISENGAGE Workflow}

\begin{warningbox}[IMPORTANT - LAC OPERATES IN TWO STAGES]
LAC follows a two-stage workflow:
\begin{enumerate}
    \item \textbf{ARM}: Press Button 2 while holding Button 3 to \textbf{arm} LAC. This latches the target's current tracking rate. Lead is NOT yet applied.
    \item \textbf{ENGAGE}: When you press the fire trigger (Button 0), LAC becomes \textbf{engaged} and lead is actively applied to the gimbal.
    \item \textbf{DISENGAGE}: When you release the fire trigger, LAC returns to \textbf{armed} state. Lead is removed, but LAC remains armed for subsequent bursts.
\end{enumerate}
\end{warningbox}

\textbf{Why Two Stages?}
\begin{itemize}
    \item Arming captures the target's motion at a specific moment
    \item Lead is only applied during actual firing
    \item Allows operator to fire multiple bursts using the same latched rate
    \item Releasing trigger removes lead but keeps LAC armed for next burst
\end{itemize}
...
\begin{itemize}
    \item When LAC is armed, system \textbf{latches} current target tracking rate
    \item Lead calculation uses latched rate, not continuously updated rate
    \item If operator disarms LAC then attempts to re-arm, must wait \textbf{2 seconds minimum}
    \item \textbf{System blocks the action:} If you attempt to re-arm LAC within 2 seconds of the last disarm, the button press is \textbf{ignored entirely}. The system will not respond until the 2-second interval has passed.
\end{itemize}
...
\begin{lstlisting}
LAC: OFF       (gray)   - LAC disabled (not armed)
LAC ARMED      (cyan)   - LAC armed, awaiting fire trigger
LEAD ANGLE ON  (green)  - LAC engaged, lead actively applied
LAC: LAG       (yellow) - Insufficient tracking data, wait 2-5 sec
LAC: ZOOM OUT  (red)    - FOV too narrow, zoom out required
\end{lstlisting}

\subsubsection{STATUS: "LAC ARMED" (CYAN)}

\textbf{Meaning:} LAC is armed but fire trigger not pressed

\textbf{Display:}
\begin{itemize}
    \item OSD shows: \osd{LAC ARMED} in cyan/blue
    \item Target tracking rate has been captured (latched)
    \item Lead angle is \textbf{NOT} being applied yet
\end{itemize}

\textbf{Operator Action:}
\begin{itemize}
    \item Lead will be applied when you press the fire trigger
    \item Fire trigger engages LAC → status changes to "LEAD ANGLE ON"
    \item Releasing trigger returns to "LAC ARMED" state
\end{itemize}
...
% ----------------------------------------------------------------------------
\subsection{LAC DISARMING PROCEDURE}

\subsubsection{When to Disarm LAC}
\begin{itemize}
...
\begin{procedurebox}[LAC DISARMING]
\textbf{DISARMING:}
\begin{enumerate}
    \item Hold \button{Button 3} (Dead Man Switch)
    \item Press \button{Button 2} (LAC Toggle)
...
\textbf{VERIFICATION:}
\begin{checklist}
    \item \osd{LAC ARMED} and \osd{LEAD ANGLE ON} indicators disappear
    \item Status returns to \osd{LAC: OFF}
    \item Reticle pipper returns to boresight alignment (with gravity drop only)
    \item System ready for stationary target engagement
\end{checklist}
\end{procedurebox}

\begin{notebox}
Remember: You must wait \textbf{2 seconds} before re-arming LAC. If you press Button 2 too soon, the command is ignored.
\end{notebox}
...
\begin{notebox}[NO OPERATOR ACTION REQUIRED]
Ballistic drop compensation is calculated and applied \textbf{automatically} whenever the LRF provides a valid range measurement. You do not need to activate any switch or menu option. The CCIP pipper always shows the gravity-compensated impact point when range is known.
\end{notebox}

\textbf{What It Corrects:}
\begin{itemize}
    \item \textbf{Gravity drop} - Bullet falls due to gravity over distance
    \item \textbf{Wind deflection} - Crosswind pushes bullet laterally (if environmental parameters set)
    \item \textbf{Environmental factors} - Temperature, altitude affect trajectory (if environmental parameters set)
\end{itemize}

\textbf{Operator Action:} \textbf{NONE REQUIRED} - System applies gravity drop corrections automatically when range is valid. Environmental corrections (temperature, altitude, wind) require configuration via menu.

\textbf{OSD Indicator:} \osd{ENV} appears when environmental parameters are active (in addition to automatic gravity drop).
...
% ----------------------------------------------------------------------------
\subsection{CCIP PIPPER DISPLAY}

The system displays two separate visual elements on the OSD:

\begin{itemize}
    \item \textbf{Reticle (Crosshair):} Shows gun boresight with zeroing only - where the gun barrel is actually pointed
    \item \textbf{CCIP Pipper:} Shows predicted impact point including all ballistic corrections
\end{itemize}

\begin{notebox}[CCIP PIPPER VISIBILITY]
The CCIP pipper appears whenever a valid LRF range exists. It shows where your rounds will actually impact, accounting for:
\begin{itemize}
    \item Gravity drop (automatic when range valid)
    \item Wind deflection (when environmental parameters set)
    \item Motion lead (when LAC is engaged during firing)
\end{itemize}
\textbf{Aim at the CCIP pipper, not the center crosshair, for accurate fire.}
\end{notebox}
...
\textbf{+ Motion Lead Compensation (only when LAC engaged during firing)}
}
}
\end{center}
