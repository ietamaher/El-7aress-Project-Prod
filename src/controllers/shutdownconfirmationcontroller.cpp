...
ShutdownConfirmationController::ShutdownConfirmationController(QObject* parent)
    : QObject(parent), m_shutdownTimer(new QTimer(this)) {
    ...
    connect(m_shutdownTimer, &QTimer::timeout, this,
            &ShutdownConfirmationController::performShutdown);
}
...
void ShutdownConfirmationController::initialize() {
    ...
    connect(m_stateModel, &SystemStateModel::colorStyleChanged, this,
            &ShutdownConfirmationController::onColorStyleChanged);
...
void ShutdownConfirmationController::onUpButtonPressed() {
    if (!m_viewModel || m_shutdownInProgress)
        return;
    ...
    qDebug() << "ShutdownConfirmationController: UP pressed, selection now:"
             << (m_viewModel->selectedOption() == ShutdownConfirmationViewModel::YesShutdown
                     ? "YES"
                     : "NO");
}
...
void ShutdownConfirmationController::onSelectButtonPressed() {
    ...
    if (selected == ShutdownConfirmationViewModel::YesShutdown) {
        ...
        qInfo() << "ShutdownConfirmationController: Shutdown confirmed, initiating shutdown "
                   "sequence...";
