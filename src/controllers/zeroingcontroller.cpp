...
ZeroingController::ZeroingController(QObject* parent) : QObject(parent) {}
...
void ZeroingController::initialize() {
    ...
    connect(
        m_stateModel, &SystemStateModel::zeroingModeChanged, this,
        [this](bool active) {
            if (!active && m_currentState == ZeroingState::Instruct_MoveReticleToImpact) {
                qDebug()
                    << "[ZeroingController] Zeroing cancelled externally during instruction phase";
                hide();
    ...
    connect(m_stateModel, &SystemStateModel::colorStyleChanged, this,
            &ZeroingController::onColorStyleChanged);
...
void ZeroingController::updateUI() {
    ...
    case ZeroingState::Completed:
        ...
        m_viewModel->setInstruction("Zeroing Adjustment Applied!\n"
