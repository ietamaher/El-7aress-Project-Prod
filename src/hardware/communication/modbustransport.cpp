...
ModbusTransport::ModbusTransport(QObject* parent)
    : Transport(parent), m_client(new QModbusRtuSerialClient(this)), m_slaveId(1)  // Default value
{
...
bool ModbusTransport::open(const QJsonObject& config) {
    ...
    qDebug() << "ModbusTransport: Connected successfully to" << port << "with slave ID"
             << m_slaveId;
    return true;
}
...
void ModbusTransport::onStateChanged(QModbusDevice::State state) {
    ...
    case QModbusDevice::ClosingState:
        ...
        break;
    default:
        stateStr = QString("Unknown(%1)").arg(state);
    }
    ...
    if (state == QModbusDevice::UnconnectedState && m_client->error() != QModbusDevice::NoError) {
        qWarning() << "ModbusTransport: Slave" << m_slaveId
                   << "connection failed:" << m_client->errorString();
    }
...
void ModbusTransport::onModbusError(QModbusDevice::Error err) {
    if (err != QModbusDevice::NoError) {
        QString errorMsg =
            QString("ModbusTransport slave %1: %2").arg(m_slaveId).arg(m_client->errorString());
